...
$(document).ready(function () {
  ...
  $("#add-client").change(function () {
    ...
    if ($(this).is(":checked")) {
      ...
      html +=
        ' <h3 class="card-title" style="margin-bottom: 15px;">Nouveau Client</h3>';
      html += '<div class="row">';
      ...
      html +=
        '<textarea class="form-control" id="client-commentaire-add" ></textarea>';
      html += "</div>";
      html += " </div>";
      html += "  </div>";
  ...
  $("#clients").change(function () {
    ...
    $.ajax({
      url: "http://webapp.saweblia.ma/adresse_client/" + $(this).val(),
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      },
      success: function (data) {
        ...
        for (i = 0; i < data.Adresses.length; i++) {
          $("#listeAdresse").append(
            "<option value='" +
              data.Adresses[i].AdressID +
              "'>" +
              data.Adresses[i].Rue +
              " " +
              data.Adresses[i].Quartier +
              "," +
              data.Adresses[i].Ville +
              "</option>"
          );
        }
      },
  ...
  $("#listeAdresse").change(function () {
    $.ajax({
      url: "http://webapp.saweblia.ma/adresses/" + $(this).val(),
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      },
      success: function (data) {
  ...
  $("#valider-prestation").click(function () {
    if (prestation.length == 0) var id = 1;
    ...
    tableau +=
      ...
      "'>" +
      $("#select-prestation").find(":selected").text() +
      "</td>";
    tableau += "<td> </td>";
    ...
    tableau +=
      "<td id='" +
      $("#listeArtisans").find(":selected").val() +
      "'> " +
      $("#listeArtisans").find(":selected").text() +
      "</td>";
    tableau +=
      '<td><button onclick="removePrestation(' +
      id +
      ')" type="button" class="btn btn-danger action"><span class="material-icons">delete_sweep</span></button><button type="button" class="btn btn-warning action" onclick="editPrestation(' +
      id +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.Coordinateurs.length; i++) {
        if (i == 0)
          $("#listeCoordinateur").append(
            "<option value='" +
              data.Coordinateurs[i].CoordinateurID +
              "' selected='true'>" +
              data.Coordinateurs[i].Nom +
              "</option>"
          );
        else
          $("#listeCoordinateur").append(
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.Prestations.length; i++) {
        if (i == 0) {
          ...
          $("#total").val(
            data.Prestations[i].PrixVente *
              0 *
              data.Prestations[i].CoefficientRemise
          );
        } else
          $("#select-prestation").append(
            "<option value='" +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.Artisans.length; i++) {
        $("#listeArtisans").append(
          "<option value='" +
            data.Artisans[i].ArtisanID +
            "'>" +
            data.Artisans[i].Nom +
            "</option>"
        );
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.Fournitures.length; i++) {
        if (i == 0) {
          $("#listeFourniture").append(
            "<option value='" +
              data.Fournitures[i].FournitureID +
              "' >" +
              data.Fournitures[i].Libelle +
              "</option>"
          );
          $("#fPUVente").val(data.Fournitures[i].PrixVente);
          $("#fquantité").val(0);
          $("#fFournisseur").html(
            data.Fournitures[i].Fournisseur.NomFournisseur
          );
