$(document).ready(function () {
  ...
  $.getJSON('http://webapp.saweblia.ma/prestations', function(data) {
    ...
    for (i = 0; i < data.Prestations.length; i++) {
        $('#select-prestation').append("<option value='"+data.Prestations[i].PrestationID+"'>" + data.Prestations[i].Libelle+ "</option>")
        } 
  })
   ...
   $('#PUAchat').val(data.PrixAchat)
    ...
    $('#add-client').change(function() {
        ...
        for (i = 0; i < data.Clients.length; i++) {
            $('#clients').append("<option value='"+data.Clients[i].ClientID+"'>" + data.Clients[i].Nom+ "</option>")
            } })
